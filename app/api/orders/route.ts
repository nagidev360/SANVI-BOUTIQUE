...
export async function POST(req:Request){try{
 ...
 const subtotal=items.reduce((s:number,i:any)=>s+Math.max(0,Number(i.price))*Math.max(1,Number(i.qty)),0); const shipping=subtotal>1999?0:99; const total=subtotal+shipping
 ...
 const rows=items.map((i:any)=>({order_id:order.id,product_id:null,quantity:Math.max(1,Number(i.qty)),unit_price:Math.max(0,Number(i.price))})); const {error:itemError}=await supabase.from('order_items').insert(rows); if(itemError)throw itemError
 if(body.paymentMethod==='razorpay'){
  if(!process.env.RAZORPAY_KEY_ID||!process.env.RAZORPAY_KEY_SECRET)return NextResponse.json({error:'Online payment is not configured yet. Please use Cash on Delivery.'},{status:503})
